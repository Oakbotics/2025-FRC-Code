...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    m_driverController
        .a()
        .onTrue(new L2ScoreCommandGroup(m_elevatorSubsystem, m_wristSubsystem)); // L2 Scoring
    m_driverController
        .y()
        .onTrue(new L4ScoreCommandGroup(m_elevatorSubsystem, m_wristSubsystem)); // L4 Scoring
    // m_driverController.y().onTrue(new WristUpCommand(m_wristSubsystem)); //L4 Scoring
    m_driverController
        .x()
        .onTrue(new L3ScoreCommandGroup(m_elevatorSubsystem, m_wristSubsystem)); // L3 Scoring
    // m_driverController.a().onTrue(new WristDownCommand(m_wristSubsystem)); // L2 Scoring
    m_driverController
        .b()
        .onTrue(new IntakeCommandGroup(m_elevatorSubsystem, m_wristSubsystem)); // Not in use

    m_driverController.povUp().onTrue(new InstantCommand(() -> m_driveSubsystem.setGyro(0)));
    m_driverController
        .povLeft()
        .whileTrue(new VelocityTuningCommand(m_driveSubsystem)); // Not in use
    m_driverController
        .povDown()
        .onTrue(new InstantCommand(() -> m_driveSubsystem.limeLightPoseUpdate()));
    // m_driverController.povRight() // Not in use

    m_driverController
        .rightBumper()
        .whileTrue(m_driveSubsystem.findPathToPose(m_driveSubsystem.findPathToPole(false), false));
    m_driverController
        .leftBumper()
        .whileTrue(m_driveSubsystem.findPathToPose(m_driveSubsystem.findPathToPole(true), false));
    ...
    // m_operatorController.a().onTrue(new L2AlgaeCommandGroup(m_elevatorSubsystem,
    // m_wristSubsystem)); // Algae Kick Out Postion L2
    // m_operatorController.x().onTrue(new L3AlgaeCommandGroup(m_elevatorSubsystem,
    // m_wristSubsystem)); // Algae Kick Out Postion L3
  ...
  public Command getAutonomousCommand() {
    ...
    return new Middle3Piece(
        m_driveSubsystem, m_elevatorSubsystem, m_wristSubsystem, m_intakeSubsytem);
  }
}
